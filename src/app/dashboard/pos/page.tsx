...
import { 
  getUnifiedPOSData, 
  getUserPOSDrafts, 
  getAllPendingPOSOrders,
  getUnifiedPOSSalesHistory,
  getUnifiedPOSAnalytics
} from "@/app/actions/unified-pos-actions"
...
export default async function UnifiedPOSPage() {
  ...
  const [posData, initialOrders, initialAnalytics, userDrafts, allPendingRes] = await Promise.all([
    getUnifiedPOSData(),
    getUnifiedPOSSalesHistory({ dateFilter: "today" }),
    getUnifiedPOSAnalytics("today"),
    getUserPOSDrafts(),
