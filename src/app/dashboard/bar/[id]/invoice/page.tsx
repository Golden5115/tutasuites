      ...
      {order.linkedRestaurantOrder && (
        <div className="max-w-[380px] mx-auto mb-4 p-3 rounded-xl bg-primary/10 border border-primary/30 flex items-center justify-between text-xs">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="font-bold text-white">Linked with Food #{order.linkedRestaurantOrder.id.slice(-6).toUpperCase()}</p>
              <p className="text-zinc-400 text-[10px]">Combined: ₦{(order.totalAmount + order.linkedRestaurantOrder.totalAmount).toLocaleString()}</p>
            </div>
          </div>
          <Link
            href={`/dashboard/combined-invoice?restaurantId=${order.linkedRestaurantOrder.id}&barId=${order.id}`}
