...
import { addRestaurantItem, updateRestaurantItem, deleteRestaurantItem } from "@/app/actions/restaurant-actions"
...
export function RestaurantCatalogClient({ items }: { items: any[] }) {
  ...
  const handleAdd = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      const res = await addRestaurantItem(new FormData(e.currentTarget))
  ...
  const handleUpdate = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setError("")
    startTransition(async () => {
      const res = await updateRestaurantItem(new FormData(e.currentTarget))
      if (res.error) setError(res.error)
  ...
  return (
    <div className="space-y-6 animate-slide-up-delay-1">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold font-heading">Menu List</h2>
      ...
      <div className="grid gap-3">
            ...
            <CardContent className="p-4">
              <form onSubmit={handleAdd} className="grid sm:grid-cols-12 gap-3 items-end">
                <div className="sm:col-span-3 space-y-1.5">
                  <Label>Name</Label>
                  <Input name="name" required placeholder="e.g. Jollof Rice" className="bg-white dark:bg-black/20" />
                </div>
                <div className="sm:col-span-2 space-y-1.5">
                  <Label>Category</Label>
                  <Input name="category" required placeholder="Rice Dishes" className="bg-white dark:bg-black/20" />
                </div>
                <div className="sm:col-span-2 space-y-1.5">
                  <Label>Price (₦) - 0 for Custom</Label>
                  <Input name="price" type="number" required placeholder="1500" className="bg-white dark:bg-black/20" />
                </div>
                <div className="sm:col-span-2 space-y-1.5">
                  <Label>Stock Qty</Label>
                  <Input name="stock" type="number" required defaultValue="50" className="bg-white dark:bg-black/20" />
                </div>
              ...
              {editingId === item.id ? (
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="grid sm:grid-cols-12 gap-4 w-full">
                    ...
                    <div className="sm:col-span-2 font-bold text-primary">
        ...
        {items.length === 0 && !isAdding && (
          <div className="text-center py-10 text-muted-foreground/60">
            No items in the catalog. Add one above.
          </div>
        )}
      </div>
    </div>
