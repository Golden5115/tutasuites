import { getRestaurantOrderById } from "@/app/actions/restaurant-actions"
...
export default async function RestaurantInvoicePage({ params }: { params: Promise<{ id: string }> }) {
          ...
          <div className="space-y-1.5 pt-1">
            {order.items.map((item) => (
              <div key={item.id} className="grid grid-cols-12 text-[11px] leading-tight">
                <span className="col-span-6 font-medium truncate pr-1">{item.item?.name || "Food Item"}</span>
